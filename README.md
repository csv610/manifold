...
This directory (`app/`) contains CLI demonstration applications that showcase the capabilities of the Manifold geometry library. **These are example applications only** - they are not part of the original Manifold library and are provided as-is for demonstration purposes.

### Building the CLI Applications

```bash
mkdir build && cd build
cmake .. -DMANIFOLD_TEST=ON
cmake --build . --target manifold_primitives manifold_boolean manifold_transform manifold_extrude manifold_smooth manifold_hull manifold_slice manifold_warp manifold_levelset manifold_info manifold_cross
```
...
| `manifold_boolean` | Boolean operations (union, subtract, intersect, Minkowski sum/difference) |
| `manifold_transform` | Affine transformations (translate, scale, rotate, mirror) |
...
| `manifold_levelset` | Implicit SDF modeling (sphere, box, torus, capsule, gyroid, schwarz) |
...
```bash
# Create primitives
./manifold_primitives cube --size=2 -o cube.obj
./manifold_primitives sphere --radius=1 -o sphere.obj
...
These CLI demo applications are provided separately from the main Manifold library. They are example code demonstrating how to use the Manifold C++ API and are not part of the original Manifold project.

**Credits:** The underlying Manifold geometry library is developed by [Emmett Lalish](https://github.com/elalish) and contributors. See the original repository at https://github.com/elalish/manifold for the full source code and documentation.
